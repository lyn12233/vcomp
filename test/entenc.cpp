/* (lyn12233) this file is copied/manipulated from [libaom] */
/*
...
// --- supp codes added: (2026.04.03, Lyn12233)
...
#define CDF_PROB_TOP (1 << 15)
#define EC_PROB_SHIFT 6
#define CDF_SHIFT 0
#define EC_MIN_PROB 4
#define OD_ICDF(x) ((1 << 15) - x)
...
void od_ec_enc_patch_initial_bits(od_ec_enc *enc, unsigned val, int nbits) {
    ...
    if (enc->offs > 0) {
        ...
        enc->precarry_buf[0] = (uint16_t)((enc->precarry_buf[0] & ~mask) | val << shift);
    } else if (9 + enc->cnt + (enc->rng == 0x8000) > nbits) {
        /*The first byte has yet to be output.*/
        enc->low = (enc->low & ~((od_ec_window)mask << (16 + enc->cnt))) | (od_ec_window)val << (16 + enc->cnt + shift);
    } else {
