...
*/
int c1enc_rdstat_compare(const c1enc_rdstat_t *a, const c1enc_rdstat_t *b);
...
typedef struct {
    ...
    uint8_t intra_try_cfl;       // try chroma-from-luma
    C1_PRED_MODE intra_rng_max;  // max intra mode (excluded) to search
    uint8_t intra_matrices_mask; // currently unused
} c1enc_search_option_t;

/** search intra mode at block level.
 measures by sad.
*/
int c1enc_search_intra_b(c1enc_block_t *b, const c1enc_search_option_t *opt);
